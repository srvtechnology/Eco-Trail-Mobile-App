...
class _SearchoptionState extends State<Searchoption> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      await homeOptionProvider.fetchPlacesByCategory(context, false,categoryId: 0);
      setState(() {
  ...
  void _onSearchChanged() {
    ...
    setState(() {
      _filteredPlaces = _allPlaces
          .where((place) => place.placeName?.toLowerCase().contains(query) ?? false)
          .toList();
    });
  }
                ...
                child: _filteredPlaces.isEmpty
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final elevation = highlightInfo.isNotEmpty
                        ? highlightInfo.first.values.first.toString()
                        : "N/A";

                    return GestureDetector(onTap: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(builder: (_) => DetailScreen(_filteredPlaces[index].id.toString(), _filteredPlaces[index])),
                      );
                    },
                      child: Container(
